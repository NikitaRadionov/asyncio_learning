...
# менеджеры. Это специальные классы, которые позволяют асинхронно захватывать ресурсы, а затем освобождать их даже при наличии исключения.
# Их задача – очистить захваченные ресурсы без лишнего кода, особенно они полезны при работе с HTTP-сеансами
...
# Эта библиотека позволяет создавать клиенты и серверы с неблокирующими сокетами.
# В случае веб-клиента мы можем конкурентно выполнять несколько запросов, не блокируя цикл событий.
#
# 3.
...
# допускающих ожидание объектов по мере их завершения.
# Она возвращает итератор по будущим объектам, который можно обойти.
# Как только сопрограмма или задача завершается, итератор отдает ее результат.
#
# 5.
# Если мы хотим выполнить несколько задач конкурентно, но при этом понимать
# какие задачи уже завершились, а какие еще работают, то можем использовать функцию wait.
# Она также дает более точный контроль над моментом возврата результатов.
# После возврата мы получаем множество завершившихся задач и множество еще работающих.
